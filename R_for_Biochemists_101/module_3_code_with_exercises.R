...
### Exercise 3 answer
mutant <- subset(enzdata, Enz=="H297F")
fit_mut <- nls(MMcurve, mutant, start=list(Vmax=50,Km=0.2))
summary(fit_mut)


Vmax_mut <- summary(fit_mut)$coefficients[1]
Km_mut <- summary(fit_mut)$coefficients[2]
paste("Vmax of H297F mutant: ", round(Vmax_mut, 1))
paste("Km of H297F mutant: ", round(Km_mut, 3))

### Exercise 4 answer
# add annotation
enz_plot_lines + 
  annotate(geom="text", x = 1.2, y = 25, label = "Km") +
  annotate(geom="text", x = 1.6, y = 25, label = "Vmax") +
  annotate(geom="text", x = 0.7, y = 20, label = "WT") +
  annotate(geom="text", x = 0.7, y = 10, label = "H297F") +
  
  annotate(geom="text", x = 1.2, y = 20, label = round(Km_WT, 3)) +
  annotate(geom="text", x = 1.6, y = 20, label = round(Vmax_WT, 1)) +
  annotate(geom="text", x = 1.2, y = 10, label = round(Km_mut, 3)) +
  annotate(geom="text", x = 1.6, y = 10, label = round(Vmax_mut, 1))
